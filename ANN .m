...
%% 读取读取 
ANN=load('ANN.txt');
ANNlabel=load('ANNIM.txt'); 
N=length(ANNlabel); %全部样本数目
temp = randperm(size(ANN,1));
%% 划分训练集、测试集  
input_train=ANN(temp(1:0.8*N),[1,2,4])';
output_train =ANNlabel(temp(1:0.8*N))';
input_test=ANN(temp(0.8*N+1:0.9*N),[1,2,4])';
output_test=ANNlabel(temp(0.8*N+1:0.9*N))';
input_valid=ANN(temp(0.9*N+1:end),[1,2,4])';
output_valid=ANNlabel(temp(0.9*N+1:end))';
%% 数据归一化（输入、输出） 
[inputn,inputps]=mapminmax(input_train,-1,1); 
[outputn,outputps]=mapminmax(output_train); 
inputn_test=mapminmax('apply',input_test,inputps);
inputn_train=mapminmax('apply',input_train,inputps);
inputn_valid=mapminmax('apply',input_valid,inputps);
%% 获取输入层节点、输出层节点个数 
inputnum=3; 
outputnum=1;
disp('/////////////////////////////////') 
